...
static const uint8_t initB[] = {
  19,                                   // 19 lệnh
  ST7735_SWRESET, DELAY, 150,           //  1: reset
  ST7735_SLPOUT,  DELAY, 255,           //  2: sleep out
  ...
  ST7735_INVCTR, 1, 0x07,               // inversion off
  ...
  ST7735_INVOFF, 0,                     // display inversion off

  ST7735_MADCTL,1, 0xC0,                // MX+MY + RGB

  ST7735_COLMOD,1, 0x05,                // 16‑bit color
  ...
  ST7735_NORON, DELAY, 10,              // normal on +10ms
  ST7735_DISPON,DELAY,100               // display on +100ms
};
...
void ST7735_Init(void) {
	ST7735_Select();
	    ...
	    // MADCTL (orientation and color order)
	    ST7735_WriteCommand(0x36);
	    uint8_t madctl = 0xC0; // MX + MY + RGB
